...
const CompanyLayout = ({
  ...
  return (
    <div className="min-h-screen flex w-full bg-background">
      {/* ====================================================================== */}
      {/* AJUSTE 1: Se fuerza el ancho del Sidebar para ambos estados          */}
      {/* ====================================================================== */}
      <Sidebar 
        variant="sidebar" 
        collapsible="icon" 
        className="data-[state=expanded]:w-80 data-[state=collapsed]:w-16 border-r bg-sidebar shadow-xl z-40"
      >
        {/* ====================================================================== */}
        {/* AJUSTE 2: Se usa "group-data-" para que el header se oculte         */}
        {/* ====================================================================== */}
        ...
        <div className="hidden group-data-[state=collapsed]:flex items-center justify-center p-4 border-b">
        ...
        <SidebarContent className="px-2 py-6 space-y-3">
          {sidebarMenuItems.map((category) => (
            <SidebarGroup key={category.category} className="p-0 space-y-2">
              <SidebarGroupLabel className="px-3 py-2 text-xs font-semibold uppercase text-sidebar-muted-foreground group-data-[state=collapsed]:hidden">
                    ...
                    return (
                      <SidebarMenuItem key={item.id} className="group-data-[state=collapsed]:flex group-data-[state=collapsed]:justify-center w-full">
                        <SidebarMenuButton
                          isActive={isActive}
                          disabled={isDisabled}
                          className={`relative group transition-all duration-300 font-medium text-sm data-[state=expanded]:p-4 data-[state=expanded]:rounded-xl data-[state=collapsed]:h-12 data-[state=collapsed]:w-12 data-[state=collapsed]:p-0 data-[state=collapsed]:rounded-full data-[state=collapsed]:flex data-[state=collapsed]:items-center data-[state=collapsed]:justify-center ${isActive ? "bg-sidebar-accent text-sidebar-accent-foreground shadow-lg" : isDisabled ? "opacity-40 cursor-not-allowed" : "hover:bg-sidebar-accent/30"}`}
                          onClick={isDisabled ? undefined : () => setActiveView(item.id)}
                          tooltip={item.label}
                        >
                          <Icon className={`transition-all duration-300 group-data-[state=expanded]:size-5 group-data-[state=collapsed]:size-6 ${isActive ? 'text-sidebar-accent-foreground' : 'text-sidebar-foreground'}`} />
        ...
        <SidebarFooter className="p-4 border-t group-data-[state=collapsed]:p-2">
          {/* Footer content aquí si es necesario */}
        </SidebarFooter>
      </Sidebar>
      
      <SidebarInset className="flex-1 overflow-hidden bg-background">
        <header className="flex h-16 shrink-0 items-center gap-2 border-b px-4 sticky top-0 z-30 bg-background/95 backdrop-blur-sm">
