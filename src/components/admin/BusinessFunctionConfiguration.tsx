...
export default function BusinessFunctionConfiguration() {
  ...
  const loadData = async () => {
    try {
      const [functionsRes, providersRes, modelsRes, apiKeysRes] = await Promise.all([supabase.from('business_function_configurations').select('*').order('function_name'), supabase.from('ai_providers').select('*').eq('is_active', true).order('display_name'), supabase.from('ai_provider_models').select('*').eq('is_available', true), supabase.from('llm_api_keys').select('*').eq('status', 'active')]);
  ...
  const saveConfiguration = async (functionId: string) => {
    ...
    try {
      ...
      const {
        error
      } = await supabase.from('business_function_configurations').update({
  ...
  return <div className="space-y-6">
      ...
      <div className="space-y-4">
        ...
        return <Card key={func.id}>
              <CardHeader className="cursor-pointer" onClick={() => setExpandedFunction(isExpanded ? null : func.id)}>
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center space-x-2">
                    ...
                    <Switch checked={func.is_active} onCheckedChange={checked => {
              ...
              {isExpanded && <CardContent className="pt-0">
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    <div className="space-y-4">
                      ...
                      <Select value={config?.selectedModel || ''} onValueChange={value => updateFunctionConfig(func.id, {
                  selectedModel: value
                })}>
                        <SelectTrigger>
                    ...
                    {config?.selectedModel && <div className="space-y-4">
                        <Label>Parámetros</Label>
                        <div className="space-y-3">
                          <div>
                            <Label className="text-xs">Temperatura: {config.modelParameters.temperature}</Label>
                            <Slider value={[config.modelParameters.temperature]} onValueChange={value => updateFunctionConfig(func.id, {
                      modelParameters: {
                        ...
                        temperature: value[0]
                          ...
                          <div>
                            ...
                            <Input type="number" value={config.modelParameters.max_tokens} onChange={e => updateFunctionConfig(func.id, {
                      modelParameters: {
                        ...config.modelParameters,
                        ...
                        <Button onClick={() => saveConfiguration(func.id)} disabled={saving} size="sm" className="w-full">
