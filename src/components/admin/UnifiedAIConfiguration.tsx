...
export default function UnifiedAIConfiguration() {
  ...
  const loadAllData = async () => {
    try {
      const [providersRes, apiKeysRes, modelsRes] = await Promise.all([supabase.from('ai_providers').select('*').order('display_name'), supabase.from('llm_api_keys').select('*').order('created_at', {
        ascending: false
  ...
  const addAPIKey = async () => {
    try {
      ...
      const {
        data,
        error
      } = await supabase.from('llm_api_keys').insert({
  ...
  const refreshProviderModels = async (provider: AIProvider) => {
    ...
    try {
      ...
      const {
        data,
        error
      } = await supabase.functions.invoke('fetch-available-models', {
        body: {
      ...
      if (data?.models && data.models.length > 0) {
        ...
        for (const modelName of data.models) {
          await supabase.from('ai_provider_models').upsert({
  ...
  const updateModelTypeAssignment = async (providerId: string, modelType: string, modelName: string) => {
    try {
      ...
      await supabase.from('ai_providers').update({
        configuration: {
          ...providers.find(p => p.id === providerId)?.configuration,
          model_types: newConfig
        }
      }).eq('id', providerId);
        ...
        <Card>
              ...
              <DialogContent className="max-w-md">
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <Input value={newProvider.name} onChange={e => setNewProvider({
                    ...newProvider,
                    name: e.target.value
                  ...
                  <div>
                    ...
                    <Input value={newProvider.display_name} onChange={e => setNewProvider({
                    ...newProvider,
                  ...
                  <div>
                    ...
                    <Textarea value={newProvider.description} onChange={e => setNewProvider({
                    ...newProvider,
                  ...
                  <div>
                    ...
                    <Input value={newProvider.base_url} onChange={e => setNewProvider({
                    ...newProvider,
                  ...
                  <div>
                    ...
                    <Input value={newProvider.env_key} onChange={e => setNewProvider({
                    ...newProvider,
                    env_key: e.target.value
          ...
          <CardContent>
            <div className="grid gap-3">
              ...
              return <div key={provider.id} className="p-4 border rounded-lg space-y-4">
                    ...
                    <div className="pl-8 space-y-3">
                      <div className="flex items-center justify-between">
                        ...
                        <Dialog open={showAPIKeyDialog && newAPIKey.provider === provider.name} onOpenChange={open => {
                          ...
                          <DialogContent className="max-w-md">
                            ...
                            <div className="space-y-4">
                              <div>
                                ...
                                <Input value={newAPIKey.api_key_name} onChange={e => setNewAPIKey({
                              ...newAPIKey,
                              ...
                              <div>
                                <Label>API Key</Label>
                                <Input type="password" value={newAPIKey.api_key} onChange={e => setNewAPIKey({
                              ...newAPIKey,
                      ...
                      {providerAPIKeys.length > 0 ? <div className="grid gap-2">
                          {providerAPIKeys.map(key => <div key={key.id} className="flex items-center justify-between p-2 bg-muted/30 rounded">
                              <div>
                        ...
                        </div> : <p className="text-sm text-muted-foreground">No hay API keys configuradas</p>}
                      ...
                      <div className="space-y-3">
                        <div className="flex items-center justify-between">
                          ...
                          <Button variant="outline" size="sm" onClick={() => refreshProviderModels(provider)} disabled={!providerAPIKeys.some(key => key.status === 'active') || loadingModels[provider.id]}>
                            <RefreshCw className={`h-3 w-3 mr-1 ${loadingModels[provider.id] ? 'animate-spin' : ''}`} />
                            {loadingModels[provider.id] ? 'Cargando...' : 'Cargar Modelos'}
                          </Button>
                        </div>
                        
                        {providerModels.length > 0 ? <div className="grid gap-4">
                        ...
                        return <div key={type.value} className="space-y-2">
                                  <h5 className="text-xs font-medium text-muted-foreground flex items-center gap-1">
                                    <span>{type.icon}</span>
                                    {type.label}
                                  </h5>
                                  <div className="pl-4">
                                    <Select value={selectedModel || 'none'} onValueChange={value => {
                              ...
                              updateModelTypeAssignment(provider.id, type.value, modelName);
                                      ...
                                      <SelectContent>
                                        ...
                                        {typeModels.map(model => <SelectItem key={model.id} value={model.model_name}>
                                            {model.display_name}
                                          </SelectItem>)}
                                         {typeModels.length === 0 && <SelectItem value="unavailable" disabled>
                            ...
                            {providerAPIKeys.length === 0 ? 'Configura una API key para cargar modelos' : 'No hay modelos cargados. Haz clic en "Cargar Modelos"'}
                          </p>}
                      </div>
                    </div>
                  </div>;
