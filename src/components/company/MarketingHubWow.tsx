...
import { 
  BarChart3, TrendingUp, Users, Heart, Plus, 
  Zap, Target, Brain, Rocket, PenTool, Network,
  Shield
} from "lucide-react";
...
const MarketingHubWow = ({ profile }: MarketingHubWowProps) => {
  ...
  const getInitialTab = () => {
    ...
    const allowed = new Set(['inicio', 'create', 'autopilot']);
  ...
  useEffect(() => {
    const resolve = async () => {
      try {
        ...
        if (import.meta.env.DEV) console.warn('Could not resolve userId:', e);
      }
    };
    resolve();
  }, [profile?.user_id]);
  ...
  const initializeMarketingHub = async () => {
    ...
    try {
      ...
      if (import.meta.env.DEV) console.error('Error initializing Marketing Hub:', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const loadConnections = async () => {
    try {
      ...
      if (import.meta.env.DEV) console.error('Error loading connections:', error);
    }
  };
  ...
  const loadRealMetrics = async () => {
    try {
      ...
      if (import.meta.env.DEV) console.error('Error loading metrics:', error);
      setRealMetrics([
  ...
  const loadPlatformStats = async () => {
    try {
      ...
      if (import.meta.env.DEV) console.error('Error loading platform stats:', error);
      setPlatformStats({
        instagram: { posts: 0, followers: 0, engagement: 0 },
        linkedin: { posts: 0, connections: 0, engagement: 0 },
        facebook: { posts: 0, likes: 0, engagement: 0 },
    ...
    <div className="space-y-6">
      ...
      <Tabs value={activeTab} onValueChange={handleTabChange} className="w-full">
        <TabsList className="flex w-full lg:w-fit mb-6">
          <TabsTrigger value="inicio" className="flex items-center gap-2">
            ...
            <span className="hidden sm:inline">{t("hub.tabs.inicio", "Inicio")}</span>
          </TabsTrigger>
          <TabsTrigger value="create" className="flex items-center gap-2">
            <Plus className="w-4 h-4" />
            <span className="hidden sm:inline">{t("hub.tabs.createPublish", "Crear + Publicar")}</span>
          </TabsTrigger>
              ...
              <CardContent className="space-y-3">
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                  <div className="p-3 rounded-lg bg-card border">
                    ...
                    <p className="text-xs text-muted-foreground">{t("hub.sdiContribution")}</p>
                  </div>
                  <div className="p-3 rounded-lg bg-card border">
                    <p className="text-2xl font-bold">{impactSummary.gapsReduced}</p>
                    <p className="text-xs text-muted-foreground">{t("hub.gapsReduced")}</p>
                  </div>
                  <div className="p-3 rounded-lg bg-card border">
                    <p className="text-2xl font-bold capitalize">{impactSummary.mostReinforcedDimension || '-'}</p>
                    <p className="text-xs text-muted-foreground">{t("hub.strongestDimension")}</p>
                  </div>
                  <div className="p-3 rounded-lg bg-card border">
                    <p className="text-2xl font-bold capitalize">{strategicContext.maturityStage}</p>
                    <p className="text-xs text-muted-foreground">{t("hub.maturityStage")}</p>
                  </div>
                </div>
              </CardContent>
            </Card>
          ...
          <Card className="overflow-hidden border shadow-sm">
            ...
            <CardContent>
              <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                ...
                <Button onClick={() => setShowConnectDialog(true)} variant="outline" className="h-auto py-4 flex-col gap-2">
                  <Network className="w-5 h-5" />
                  <span className="text-sm">{t("hub.connectNetworks")}</span>
                </Button>
      ...
      <Dialog open={showConnectDialog} onOpenChange={(open) => {
        ...
        <DialogContent className="max-w-2xl max-h-[85vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>{t("hub.connectNetworksTitle")}</DialogTitle>
            <DialogDescription>{t("hub.connectNetworksDesc")}</DialogDescription>
          </DialogHeader>
