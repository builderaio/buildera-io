...
-- Insert sample usage data with proper date distribution
WITH api_keys AS (
  SELECT id, provider, model_name, ROW_NUMBER() OVER () as rn 
...
WHERE (ak.rn + EXTRACT(day FROM ds.usage_date))::integer % 3 = 0; -- Only insert for some days to avoid too much data
