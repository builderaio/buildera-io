-- Asegurar que campos de empresa van vacíos en registros sociales
...
BEGIN
  ...
  VALUES (
    ...
    -- Para registro social, company_name va vacío (se completa en CompleteProfile)
    CASE 
      WHEN is_social_signup THEN NULL
      ELSE NEW.raw_user_meta_data->>'company_name'
    END,
    -- Para registro social, website_url va vacío (se completa en CompleteProfile)
    CASE 
      WHEN is_social_signup THEN NULL
      ELSE NEW.raw_user_meta_data->>'website_url'
    END,
    -- Para registro social, industry va vacío (se completa en CompleteProfile)
    CASE 
      WHEN is_social_signup THEN NULL
      ELSE NEW.raw_user_meta_data->>'industry_sector'
    END,
