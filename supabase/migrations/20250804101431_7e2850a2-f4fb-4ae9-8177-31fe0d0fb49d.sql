...
-- Corregir la función handle_new_user sin usar el tipo user_type en las declaraciones
...
DECLARE
  ...
  computed_user_type TEXT; -- Cambiar a TEXT en lugar de user_type
BEGIN
  ...
  ELSIF user_type_value IS NOT NULL AND user_type_value IN ('company', 'developer', 'expert') THEN
    computed_user_type := user_type_value; -- Usar como TEXT
  ELSE
    computed_user_type := 'company'; -- Default para email
  END IF;
  ...
  VALUES (
    NEW.id,
    NEW.email,
    computed_user_type::user_type, -- Cast aquí donde es seguro
    auth_provider_value,
  ...
  IF NOT is_social_signup AND COALESCE(user_type_value, 'company') = 'company' THEN
    DECLARE
      new_company_id UUID;
    BEGIN
