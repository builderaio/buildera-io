-- Migrar company_strategy para usar company_id en lugar de user_id
ALTER TABLE public.company_strategy 
ADD COLUMN company_id UUID REFERENCES public.companies(id) ON DELETE CASCADE;

-- Migrar datos existentes (asociar estrategias con la empresa principal del usuario)
...
-- Actualizar políticas RLS para usar company_id
DROP POLICY IF EXISTS "Users can create their own strategy" ON public.company_strategy;
DROP POLICY IF EXISTS "Users can delete their own strategy" ON public.company_strategy;
DROP POLICY IF EXISTS "Users can update their own strategy" ON public.company_strategy;
DROP POLICY IF EXISTS "Users can view their own strategy" ON public.company_strategy;

-- Nuevas políticas basadas en company_id
