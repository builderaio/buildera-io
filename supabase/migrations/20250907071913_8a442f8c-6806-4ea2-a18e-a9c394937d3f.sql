-- Create table for social activity analysis
...
CREATE POLICY "Users can view their own activity analysis" 
ON public.social_activity_analysis 
FOR SELECT 
USING (auth.uid() = user_id);

CREATE POLICY "Users can insert their own activity analysis" 
ON public.social_activity_analysis 
FOR INSERT 
WITH CHECK (auth.uid() = user_id);

CREATE POLICY "Users can update their own activity analysis" 
ON public.social_activity_analysis 
FOR UPDATE 
USING (auth.uid() = user_id);

CREATE POLICY "Users can delete their own activity analysis" 
ON public.social_activity_analysis 
FOR DELETE 
USING (auth.uid() = user_id);
